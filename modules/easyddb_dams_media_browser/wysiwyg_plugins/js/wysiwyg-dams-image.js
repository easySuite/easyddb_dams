...
Drupal.wysiwyg.plugins.dams_image = {
  ...
  invoke: function (data, settings, instanceId) {
    if (data.format == 'html') {
      Drupal.media.popups.mediaBrowser(function (mediaFiles) {
        Drupal.wysiwyg.plugins.dams_image.mediaBrowserOnSelect(mediaFiles, instanceId);
      }, settings['global']);
    }
  },
  ...
  mediaBrowserOnSelect: function (mediaFiles, instanceId) {
    ...
    Drupal.media.popups.mediaStyleSelector(mediaFile, function (formattedMedia) {
      Drupal.wysiwyg.plugins.dams_image.insertMediaFile(mediaFile, formattedMedia.type, formattedMedia.html, formattedMedia.options, Drupal.wysiwyg.instances[instanceId]);
    }, options);
  ...
  insertMediaFile: function (mediaFile, viewMode, formattedMedia, options, wysiwygInstance) {
    ...
    var inlineTag = Drupal.wysiwyg.plugins.dams_image.createTag(imgElement);
  ...
  addImageAttributes: function (imgElement, mediaFile, view_mode, additional) {
    ...
    this.forceAttributesIntoClass(imgElement, mediaFile, view_mode, additional);
    if (additional) {
      for (k in additional) {
        if (additional.hasOwnProperty(k)) {
          if (k == 'alt' || k === 'title') {
            imgElement.attr(k, decodeURI(additional[k]));
          }
          else if (k === 'attr') {
    ...
    if (matches) {
      ...
      for (i = 0; i < matches.length; i++) {
        ...
        if (tagmap[inlineTag]) {
          ...
          if ($.inArray(mediaObj.view_mode, ["easyddb_dams_media_browser_inline", "easyddb_dams_media_browser_download_link"])) {
            var imgElement = $(mediaMarkup);
            this.addImageAttributes(imgElement, mediaObj, mediaObj.view_mode);
            var toInsert = this.outerHTML(imgElement);
  ...
  detach: function (content, settings, instanceId) {
    ...
    if (matches = content.match(/<img[^>]+class=([\'"])media-image[^>]*>/gi)) {
      for (var i = 0; i < matches.length; i++) {
        ...
        var inlineTag = Drupal.wysiwyg.plugins.dams_image.createTag($(imageTag));
   ...
   */
  forceAttributesIntoClass: function (imgElement, mediaFile, view_mode, additional) {
    ...
    if (mediaFile.fid) {
      imgElement.addClass('img__fid__' + mediaFile.fid);
    }
    if (view_mode) {
      imgElement.addClass('img__view_mode__' + view_mode);
    }
    // Handle title and alt attributes on new file insertion.
    if (mediaFile.field_image_alt_text != undefined && mediaFile.field_image_alt_text.und) {
      additional.alt = encodeURI(mediaFile.field_image_alt_text.und[0].safe_value);
    }
    if (mediaFile.field_image_title_text != undefined && mediaFile.field_image_title_text.und) {
      additional.title = encodeURI(mediaFile.field_image_title_text.und[0].safe_value);
    }
    // Handle title and alt attributes on existing file change.
    if (mediaFile.alt != undefined) {
      additional.alt = encodeURI(mediaFile.alt);
    }
    if (mediaFile.title != undefined) {
      additional.title = encodeURI(mediaFile.title);
    }
    if (additional) {
   ...
   *   'img', depeneasyddb on whether it is an image attribute or should
  ...
  getAttributesFromClass: function (classString) {
    ...
    for (var index = 0; index < classes.length; index++) {
      ...
      if (matches && matches.length === 4) {
        // Decode title and alt values.
        if (matches[2] == 'alt' || matches[2] == 'title') {
          matches[3] = decodeURI(matches[3]);
        }
