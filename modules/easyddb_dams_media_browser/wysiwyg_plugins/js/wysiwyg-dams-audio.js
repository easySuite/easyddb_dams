...
Drupal.wysiwyg.plugins.dams_audio = {
  ...
  invoke: function (data, settings, instanceId) {
    if (data.format == 'html') {
      Drupal.media.popups.mediaBrowser(function (mediaFiles) {
        Drupal.wysiwyg.plugins.dams_audio.mediaBrowserOnSelect(mediaFiles, instanceId);
      }, settings['global']);
    }
  },
  ...
  mediaBrowserOnSelect: function (mediaFiles, instanceId) {
    ...
    Drupal.media.popups.mediaStyleSelector(mediaFile, function (formattedMedia) {
      Drupal.wysiwyg.plugins.dams_audio.insertMediaFile(mediaFile, formattedMedia.type, formattedMedia.html, formattedMedia.options, Drupal.wysiwyg.instances[instanceId]);
    }, options);
  ...
  insertMediaFile: function (mediaFile, viewMode, formattedMedia, options, wysiwygInstance) {
    ...
    if (viewMode === 'easyddb_dams_download_icon' || viewMode === 'easyddb_dams_popup') {
      imgElement = $('<img src="' + Drupal.settings.easyddb_dams.icon_path + 'doc_mp3.png"/>');
    }
    ...
    else if (viewMode === 'easyddb_dams_inline') {
...
type="application/x-shockwave-flash" flashvars="mp3=' + mediaFile.url + '&showvolume=1">\
    ...
    var inlineTag = Drupal.wysiwyg.plugins.dams_audio.createTag(imgElement);
  ...
  addImageAttributes: function (imgElement, fid, view_mode, additional) {
    ...
    imgElement.addClass('easyddb-dams-audio');
    ...
    if (matches) {
      ...
      for (i = 0; i < matches.length; i++) {
        ...
        if (tagmap[inlineTag]) {
          ...
          if (mediaObj.hasOwnProperty('attributes') && mediaObj['attributes']['class'].indexOf('easyddb-dams-audio') != -1) {
            var imgElement = $('<img src="' + Drupal.settings.easyddb_dams.icon_path + 'doc_mp3.png"/>');
  ...
  detach: function (content, settings, instanceId) {
    ...
    if (matches = content.match(/<[^>]+class=([\'"])*easyddb-dams-audio[^>]*>*<\/[^>]+>/gi)) {
      for (var i = 0; i < matches.length; i++) {
        var imageTag = matches[i];
        var inlineTag = Drupal.wysiwyg.plugins.dams_audio.createTag($(imageTag));
