...
Drupal.wysiwyg.plugins.dams_document = {
  ...
  invoke: function (data, settings, instanceId) {
    if (data.format == 'html') {
      Drupal.media.popups.mediaBrowser(function (mediaFiles) {
        Drupal.wysiwyg.plugins.dams_document.mediaBrowserOnSelect(mediaFiles, instanceId);
      }, settings['global']);
    }
  },
  ...
  mediaBrowserOnSelect: function (mediaFiles, instanceId) {
    ...
    Drupal.media.popups.mediaStyleSelector(mediaFile, function (formattedMedia) {
      ...
      if (formattedMedia.type === 'easyddb_dams_media_browser_download_icon') {
        formattedMediaType = 'easyddb_dams_media_browser_download_icon';
  ...
  addImageAttributes: function (imgElement, fid, view_mode, additional) {
    ...
    imgElement.addClass('easyddb-dams-doc');
    ...
    if (matches) {
      ...
      for (i = 0; i < matches.length; i++) {
        ...
        if (tagmap[inlineTag]) {
          ...
          if (mediaObj.view_mode == 'easyddb_dams_media_browser_download_icon') {
  ...
  detach: function (content, settings, instanceId) {
    ...
    if (matches = content.match(/<img[^>]+class=([\'"])easyddb-dams-doc[^>]*>/gi)) {
    ...
    for (i=0; i< imgElement.attributes.length; i++) {
      ...
      if (attr.specified == true) {
        if (attr.name !== 'class') {
          ...
          sorter.push({'name': attrName, 'value': attr.value});
        }
        else {
