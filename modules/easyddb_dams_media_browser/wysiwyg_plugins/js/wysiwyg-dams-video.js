...
Drupal.wysiwyg.plugins.dams_video = {
  ...
  invoke: function (data, settings, instanceId) {
    if (data.format == 'html') {
      Drupal.media.popups.mediaBrowser(function (mediaFiles) {
        Drupal.wysiwyg.plugins.dams_video.mediaBrowserOnSelect(mediaFiles, instanceId);
      }, settings['global']);
    }
  },
  ...
  mediaBrowserOnSelect: function (mediaFiles, instanceId) {
    ...
    Drupal.media.popups.mediaStyleSelector(mediaFile, function (formattedMedia) {
      Drupal.wysiwyg.plugins.dams_video.insertMediaFile(mediaFile, formattedMedia.type, formattedMedia.html, formattedMedia.options, Drupal.wysiwyg.instances[instanceId]);
    }, options);
  ...
  insertMediaFile: function (mediaFile, viewMode, formattedMedia, options, wysiwygInstance) {
    ...
    var inlineTag = Drupal.wysiwyg.plugins.dams_video.createTag(imgElement);
  ...
  addImageAttributes: function (imgElement, fid, view_mode, additional) {
    ...
    imgElement.addClass('easyddb-dams-video');
  ...
  detach: function (content, settings, instanceId) {
    ...
    if (matches = content.match(/<[^>]+class=([\'"])*easyddb-dams-video[^>]*>*<\/[^>]+>/gi)) {
      for (var i = 0; i < matches.length; i++) {
        var imageTag = matches[i];
        var inlineTag = Drupal.wysiwyg.plugins.dams_video.createTag($(imageTag));
